...
# ---------------------------------------parameters-------------------------------------
numbertask =119
(ALPHA, BETA, LAMDA, RHO, Q) = (2.0,1.0,3.0,0.5,100.0)
...
# Input test_task list
Task_list = pd.read_excel('Test_Task_list.xlsx', sheet_name='Tasklist', usecols=[0,3,6,7,8,9,10,11], skiprows=0, nrows=numbertask, dtype=object)
Task_list_Forklift = Task_list.loc[Task_list['Task type'] == 'C04_CMD']
Task_list_AGV = Task_list.loc[Task_list['Task type'] != 'C04_CMD']
# input visibility graph for scheduling
visibility_graph=pd.read_excel('Test_Visibility_graph.xlsx', sheet_name='Sheet1', usecols="B:EK", skiprows=0, nrows=120, dtype=object)
visibility_graph=visibility_graph.values
...
scheduling_result={}
scheduling_result=Task_assignment.best_vehicle_tasklist
...
pickle_file = open("TestVisibilityGraphACORouting_15.pkl", "rb")
...
Task_list_routing = pd.read_excel('Test_Task_list.xlsx', sheet_name='Tasklist_routing', index_col=0,usecols=[0,3,6,7,8,9,10,11], skiprows=0, nrows=numbertask+1,dtype=object)
#----------------------------Routing parameters------------------------------------
...
(node_num, ant_num_routing) = (len(nodes),50)
...
class ACO_Routing(object):
    ...
    def search_path(self, graph, scheudling_list):
        ...
        while self.__running:
            ...
            for task_location in range(len(scheudling_list[5])-1):
                for vehicle in range(6):
                    if scheudling_list[vehicle][task_location] != -1 and task_location < (len(scheudling_list[5])-1):
                        #print(task_location)
                        self.new()
                    ...
                    elif scheudling_list[vehicle][task_location] != -1 and task_location == len(scheudling_list[5])-1:
                        ...
                        for ant in self.ants:
                            ...
                            ant.current_node = Task_list_routing.loc[scheudling_list[vehicle][task_location], 'End node']
                            ant.path.append(ant.current_node)
                            target_node = Task_list_routing.loc[scheudling_list[vehicle][task_location], 'Start node']
                        ...
                        #print(scheudling_list[vehicle][task_location])
...
if __name__ == '__main__':
    ...
    NumberofLinkconflicts=Routing.vehicleNY
    NumberofNodeconflicts = Routing.vehicleNZ
